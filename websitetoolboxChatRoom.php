...
function wtb_chatroom_api_option($args) {
	...
	$html .= '<label for="wtb_chatroom_api"> Get your <a href="https://www.websitetoolbox.com/support/252" target="_blank">API key</a>.</label>';
...
function wtbchatroom_options_page() {
	if($_POST) {
		...
		if($_POST['wtb_chatroom_username'] && $_POST['wtb_chatroom_api']) {
			...
			$fields = array('action' => 'updateChatRoomSSOURLs','chatRoomUsername' => $_POST['wtb_chatroom_username'], 'chatRoomApikey' => $_POST['wtb_chatroom_api'], 'loginPageUrl' => $chatRoom_login_url, 'registrationPageUrl' => $chatRoom_register_url, 'chatroom_embed_page_url' => $chatroom_embed_page_url);
			...
			if(get_option("wtb_chatroom_redirect") == 1) {
				...
				wp_update_post($page);
				$page->post_content = '<script type="text/javascript" id="embedded_chatroom" src="'.$wtb_chat_embed_url.'/js/embed.js"></script><noscript><a href="'.$wtb_chat_url.'">Chat Room</a></noscript>';
				wp_update_post($page);
...
function wtbChatRoom_warning() {
	if(!get_option("wtb_chatroom_username") || !get_option("wtb_chatroom_api") || !get_option("wtb_chatroom_url")) {
		...
		";
	}  else if($_SESSION['wtb_chatroom_account_settings']) {
		if($_SESSION['wtb_chatroom_account_settings'] == 1) {
		...
		} else if($_SESSION['wtb_chatroom_account_settings'] == 2) {
	...
	if(!is_wp_error( $response_array )) {
		...
		if($response->{'success'}) {
			...
			if(!empty($_POST['rememberme'])) {
				setcookie('wtbchat_login_remember', 1, time() + 1296000, COOKIEPATH, COOKIE_DOMAIN);
			}
...
function wtbUpdateChatRoomInfo($userid, $old_user_data) {
	...
	$fields = array('apikey'=>$chatroom_api,'user'=>$old_email,'email'=>$email,'password'=>$password);
...
function changeChatRoolLink($items, $post){
	...
	if(is_user_logged_in()) {
		if($post == get_option('wtbChatRoomPageid') && get_option("wtb_chatroom_redirect") == '') {
			...
			if($_COOKIE['wtbchat_logout_token']) {
				$access_token = $_COOKIE['wtbchat_logout_token'];
			}
			if(isset($_COOKIE['wtbchat_login_remember'])) {
				$remember = 1;
			}
			$add_token = array('access_token' => $access_token, 'rememberMe' => $remember);
...
function updateChatRoomPageContent($content) {
	...
	if($GLOBALS['post']->ID == get_option('wtbChatRoomPageid') && get_option("wtb_chatroom_redirect") != '' && is_user_logged_in()) {
		...
		$content = '<script type="text/javascript" id="embedded_chatroom" src="'.$wtb_chat_embed_url.'/js/embed.js'.$access_token.'"></script><noscript><a href="'.$wtb_chat_url.'">Chat Room</a></noscript>';
	}
    return $content;
}
